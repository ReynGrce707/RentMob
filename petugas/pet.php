...
<body>
    <div class="container-fluid position-relative d-flex p-0">
        ...
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-secondary navbar-dark">
                ...
                <div class="navbar-nav w-100">
                    <a href="pet.php" class="nav-item nav-link active"><i class="fa fa-tachometer-alt me-2"></i>Dashboard</a>
        ...
        <div class="content">
            ...
            <!-- Sales Chart Start -->
            <div class="container-fluid pt-4 px-4">
                <a href="addmbl.php" class="btn btn-square btn-primary m-2"><i class="fa fa-plus"></i></a>
            </div>
            <!-- Sales Chart End -->
    ...
    <div class="row g-4">
        ...
        if ($brand != "") {
            $stmt = $koneksi->prepare("SELECT * FROM tb_mobil WHERE brand = ?");
            $stmt->bind_param("s", $brand);
            $stmt->execute();
            $result = $stmt->get_result();
        } else {
            $result = $koneksi->query("SELECT * FROM tb_mobil");
        }
        ...
        if ($result->num_rows > 0) {
                ...
                <div class="col-sm-12 col-md-6 col-xl-4">
                    <div class="h-100 bg-secondary rounded p-4">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <h6 class="mb-0"><?php echo $row["brand"]; ?></h6>
                            <small><?php echo $row["tahun"]; ?></small>
                        </div>
                        <div class="text-center">
                            <!-- Square image -->
                            <img src="../img/<?php echo $row['foto']; ?>" alt="<?php echo $row['brand']; ?>" style="width: 100%; height: 300px; object-fit: cover;" class="rounded mb-3">
                        </div>
                        <div>
                            <span>
                                <strong>Nomor Polisi:</strong> <?php echo $row["nopol"]; ?><br>
                                <strong>Harga:</strong> <?php echo number_format($row["harga"], 0, ',', '.'); ?><br>
                                <strong>Status </strong> <?php echo $row["status"]; ?><br><br>
                                <a href="editmbl.php?nopol=<?php echo $row['nopol']; ?>" class="btn btn-primary rounded-pill m-2">Edit</a>
                                <a href="del.php?nopol=<?php echo $row['nopol']; ?>" class="btn btn-primary rounded-pill m-2">Hapus</a>                                   </span>
                        </div>
                    </div>
                </div>
                <?php
            ...
            echo '<div class="col-12"><p class="text-center">Tidak ada mobil yang tersedia.</p></div>';
        }
        ?>
    </div>
